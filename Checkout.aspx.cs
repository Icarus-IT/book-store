...
namespace BookStore
{
    public partial class Checkout : System.Web.UI.Page
    {
        ...
        protected void btnOrder_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                conn = new SqlConnection(@"Data Source=DESKTOP-8M2NJ75\SQLEXPRESS;Initial Catalog=BookStore;Integrated Security=SSPI");
                conn.Open();
                ...
                com.Parameters["@registerDate"].Value = DateTime.Now ;
                com.ExecuteScalar();
                ...
                {
                    Label labelID = (Label)DataList1.Items[i].FindControl("lblID");
                    com = new SqlCommand("select quantity from Book where bookId = @bookId", conn);
                    com.Parameters.Add("@bookId", SqlDbType.Int);
                    com.Parameters["@bookId"].Value = labelID.Text;
                    dr = com.ExecuteReader();
                    dr.Read();
                    ...
                    com = new SqlCommand("update Book set quantity = @quantity where bookId = @bookId", conn);
                    com.Parameters.Add("@bookId", SqlDbType.Int);
                    com.Parameters["@bookId"].Value = labelID.Text;
                    labelID = (Label)DataList1.Items[i].FindControl("lblQuantity");
                    int quantityInDB = int.Parse(labelID.Text);
                    int remainQuantity = quantity - quantityInDB;
                    com.Parameters.Add("@quantity", SqlDbType.Int);
                    com.Parameters["@quantity"].Value = remainQuantity;
